...
const MOCK_PROMOTIONS: Omit<Promotion, 'id'>[] = [
    ...
    { code: 'DIWALI20', type: 'Percentage', value: 20, description: 'Diwali special - 20% off', status: 'Expired', platform: 'Personalized', usageLimit: 2000, usageCount: 1987, expiresAt: new Date('2023-11-15'), conditions: [], visibleOnPlatform: true, appliesTo: { products: [], categories: ['food-drink'], vendors: [] } },
];
...
export async function getTargetableItems(): Promise<{ products: TargetableItem[], categories: TargetableItem[], vendors: TargetableItem[] }> {
    ...
    return {
        ...
        categories: categoriesSnap.docs.map(doc => ({ id: doc.data().slug, name: doc.data().name, image: doc.data().image })),
