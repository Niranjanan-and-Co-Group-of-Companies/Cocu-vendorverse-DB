...
const VENDOR_ID = 'vendor001'; // In a real app, this comes from auth context.
...
export default function VendorMessagesPage() {
  ...
  React.useEffect(() => {
    const unsubscribe = onVendorConversationsUpdate(VENDOR_ID, (updatedConversations) => {
        const personalConversations = updatedConversations.filter(c => c.type !== 'Corporate');
        setConversations(personalConversations);
        setLoading(false);

      if (selectedConversation) {
        const updatedSelected = personalConversations.find(c => c.id === selectedConversation.id);
        setSelectedConversation(updatedSelected || null);
      } else if (personalConversations.length > 0) {
        // Select the first conversation by default
        setSelectedConversation(personalConversations[0]);
      }
    });
        ...
        {loading && conversations.length === 0 ? (
            ...
            <>
                ...
                <div className="hidden md:flex w-2/3 h-full flex-col">
                    <VendorChatView
                        key={selectedConversation?.id} // Force re-mount on selection change
