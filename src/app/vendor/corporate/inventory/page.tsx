...
function InventoryTable() {
    ...
    React.useEffect(() => {
        const unsubscribe = onVendorProductsUpdate(VENDOR_ID, (allProducts) => {
            // Filter for corporate products (MOQ > 1)
            const corporateProducts = allProducts.filter(p => p.moq && p.moq > 1);
            setProducts(corporateProducts);
            setLoading(false);
        });
        return () => unsubscribe();
    }, [VENDOR_ID]);
...
export default function CorporateInventoryPage() {
    return (
        <div className="flex flex-col gap-6">
            <div>
                <h1 className="text-2xl font-bold">Corporate Inventory</h1>
                <p className="text-muted-foreground">Quickly update stock levels for your B2B products.</p>
            </div>
            <InventoryTable />
        </div>
    );
