...
export default function VendorCorporateOrdersPage() {
    ...
    React.useEffect(() => {
        const unsubscribe = onVendorOrdersUpdate(VENDOR_NAME, (orders) => {
            // Filter for orders that contain B2B products (MOQ > 1)
            const corporateOrders = orders.filter(order => 
                order.items.some(item => item.vendor === VENDOR_NAME && (item.moq || 0) > 1)
            );
            setAllOrders(corporateOrders);
            setLoading(false);
        });
        return () => unsubscribe();
    }, []);
    ...
    return (
        <div className="flex flex-col gap-6">
            <div>
                <h1 className="text-2xl font-bold">Manage Corporate Orders</h1>
                <p className="text-muted-foreground">
                    View and fulfill all of your B2B and bulk orders.
                </p>
            </div>
